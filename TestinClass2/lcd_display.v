...
localparam IMG_WIDTH  = 11'd100;  // 图片宽度
localparam IMG_HEIGHT = 11'd100;  // 图片高度
...
// 由于ROM有1周期延迟，需要提前计算
assign rom_addr = (rel_y[6:0] * 7'd100) + rel_x[6:0];
  ...
  if (!sys_rst_n) begin
    ...
    if (switches == 4'b1000) begin
      ...
      if (move_cnt >= MOVE_DELAY - 1) begin
        ...
        if (vel_y < 12'sd0) begin
          // 向上移动中
          if (img_y + vel_y <= INIT_Y - MOVE_DISTANCE) begin
            // 到达上边界，反转向下
            vel_y <= 12'sd1;
