...
// 初始位置参数
parameter INIT_X = 12'd200;  // 初始X坐标
parameter INIT_Y = 12'd200;  // 初始Y坐标

// 移动参数
parameter MOVE_DISTANCE = 12'd120;  // 移动距离（像素）
parameter PIXELS_PER_SEC = 12'd70;  // 速度：70像素/秒

// 颜色定义
localparam COLOR_BG = 24'hFFFF00;  // 黄色背景

// 时钟频率参数 (lcd_clk = 33MHz)
// 每秒移动70像素，所以每移动1像素需要的时钟周期: 33MHz / 70 ≈ 471428
localparam MOVE_DELAY = 26'd471_428;  // 每像素移动延迟
  ...
  if (!sys_rst_n) begin
    ...
    if (switches == 4'b1000) begin
      ...
      if (move_cnt >= MOVE_DELAY - 1) begin
        ...
        // X方向碰壁反弹
        if (next_x + $signed(IMG_WIDTH) >= $signed(H_DISP)) begin
          // 碰到右边界，反弹向左
          vel_x <= -12'sd1;
          img_x <= $signed(H_DISP) - $signed(IMG_WIDTH) - 12'sd1;
        end else if (next_x <= 12'sd0) begin
          // 碰到左边界，反弹向右
          vel_x <= 12'sd1;
          img_x <= 12'sd1;
        end else begin
